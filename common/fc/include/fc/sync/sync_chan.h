...
public:
  sync_chan(bool time_out = true) : time_out_check(time_out) {
    if (::pipe(_pipe_sign) != 0)
      LOG_FATAL("init sync_chan pipe sign error!");
...
private:
  ...
  void m_read(CHAN_VALUE &data) {
    wait();
    {
      ...
      if (chan_queue.size() == 0)
        LOG_FATAL("empty chan queue!"); /* an error accured */
  ...
  inline void wait(const int &num = 1) {
    ...
    do {
      ...
      if (rv == -1)
        LOG_FATAL("sync_chan select read sign error!"); /* an error accured */
      else if (rv == 0)
        if (time_out_check)
          LOG_FATAL("sync_chan read sign time out error!"); /* a timeout occured */
        else
          continue;
      else
